...
export default function ZeekyPage() {
  ...
  const openApple = useCallback((track: string, artist: string) => {
    showToast(`Opening "${track}" in Apple Music\u2026`);
    window.open(`https://music.apple.com/search?term=${encodeURIComponent(track+" "+artist)}&at=1010lZGl`, "_blank");
  }, [showToast]);
      ...
      <div className="app">
        ...
        {mode==="b2b" && <div className="demo-card">
          ...
          {showResults && !isAnalyzing && <div className="results">
            ...
            <div className="results-footer"><button className="footer-btn apple" onClick={()=>openApple(sample.track,sample.artist)}><svg viewBox="0 0 24 24" fill="currentColor">{APPLE_ICON}</svg>Open in Apple Music</button><button className="footer-btn api" onClick={()=>{navigator.clipboard?.writeText(`curl https://api.zeeky.fm/v1/dna/recommend -H "Authorization: Bearer $ZEEKY_KEY" -d '{"seed_track":"isrc:${sample.isrc}","limit":25}'`).then(()=>showToast("API call copied"));}}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>Copy API Call</button></div>
          </div>}
        </div>}
